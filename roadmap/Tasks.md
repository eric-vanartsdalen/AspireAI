...
### Ingestion Trigger & Query Readiness (P1/P2)



> Verified 2026-03-26: the current upload flow stops after the file is copied to shared storage and a `files.status = 'uploaded'` row exists. Docling and LightRAG only run after an explicit call to `/processing/process-document/{id}` or `/processing/process-all`. There is no startup poller, filesystem watcher, or C#→Python post-upload call today.



- [ ] Decide the trigger contract: add a Web/API action that calls the Python processing endpoints, or implement a real automatic worker after upload
...
- [ ] Persist or expose per-document LightRAG handoff / scan state so later query failures can be traced without log scraping

- [ ] Document unsupported hot-folder behavior: a file copied into the shared folder without a matching `files` row and processing call is ignored
...
- [ ] **Upload-to-processing happy path test** — must include the trigger step (not just upload + DB row)
...
- [ ] **Python integration: upload → trigger → process → `document_pages` rows** (includes explicit HTTP call to processing endpoint)


- [ ] RAG integration: retrieval returns source-bearing results


- [ ] **E2E smoke: upload → trigger → process → chat question → citation appears** (the trigger step is now explicit)
...
| T | Upload flow makes the processing trigger explicit (manual endpoint call or proven automatic worker) | ❌ |
...
- [ ] Explicitly trigger Python processing via `/processing/process-document/{id}` or `/processing/process-all`; upload alone is not a Docling / LightRAG trigger


- [ ] Verify the triggered row is selected even if status arrives as `Uploaded`
...
- [ ] If validating a folder-drop scenario, create the companion `files` row first or expect no ingestion
...
- **[CRITICAL — Ingestion Gap]** No automatic trigger exists from upload to processing. C# writes `files` row + copies file → dead end. Python processing only fires via explicit HTTP POST to `/processing/process-document/{id}` or `/processing/process-all`. The `FlowEndToEnd` test stops at upload and never invokes processing. This is the #1 blocker for the observable pipeline.
...
- **[Trigger Contract]** The Web upload flow currently stops at filesystem copy + `files.status='uploaded'`; nothing in the Web UI, AppHost, or Python startup path automatically calls `/processing/process-document/{id}` or `/processing/process-all`


- **[Hot Folder Assumption]** A file copied directly into `data/` or `data/uploads/` without a matching `files` row is ignored because Python discovers work from SQLite state, not filesystem scans


- **[Queue Visibility]** Operators cannot currently see backlog or stuck work without inspecting SQLite rows manually; expose counts for `uploaded`, `error`, and stale `processing`


- **[LightRAG Scan Visibility]** LightRAG scan responses are attached to in-memory processing metadata during the request, but there is no durable per-file scan status for later query debugging
