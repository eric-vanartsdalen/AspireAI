...
> **Note (2026-04-22T07:50:03Z):** Merged 6 inbox decisions from three-chat-modes implementation session (Jeff, Buster, Bob). Key outcomes: (1) Chat mode compatibility strategy — `Simple` mode added as new default (local Ollama path), `Enhanced` mapped to legacy `"regular"` persisted value (backward compatible), `Critique` unchanged. (2) Three-mode acceptance gate — Four seams validated together (UI default, labels, routing split, contract default); initial artifact rejected by Buster for missing contract default, escalated to Bob for independent revision. (3) Contract default fix — `BrainChatRequest.Mode` now defaults to `Simple`, enforcing product rule at API boundary. (4) Routing logic — `Simple` uses direct local chat path; `Enhanced` and `Critique` use BRAIN gateway path. (5) Backward compatibility — Existing `"regular"` conversations load as `Enhanced` without data migration. (6) Tenant menu-close seam — Sidebar close managed by `MainLayout` on tenant switch; test seeding via real `TenantManagementService` (not reflection shortcuts). (7) Regression validation — 68+ tests passing, mode persistence/loading/routing/seeding all green; no further blockers. Merged decisions: (a) Jeff's chat-mode compatibility strategy, (b) Buster's acceptance gate with contract default requirement, (c) Bob's focused contract fix, (d) Jeff's PR #18 merge conflict resolution (IndexingStatus preserved), (e) Jeff's tenant menu-close decision, (f) Buster's tenant seeding pattern. No exact duplicates found; all six distinct in scope. Inbox cleared.
